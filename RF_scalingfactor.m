...
%% Prep
folder='/home/hannelore/Documents/ANALYSES/TVB_global2/subjects/';
cd(folder);
sublist=dir('*'); 
sublist=sublist(3:37,:);
m=length(sublist);
...
for i = 1:size(response,1)
    ...
    b_avg(:,i) = mean(cat(2,response{i,:}),2);
	B = cat( 1, B, mean( b_all ./ repmat(b_avg(:,i),1,35) ) );
end
